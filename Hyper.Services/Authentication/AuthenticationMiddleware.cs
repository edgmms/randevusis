...
namespace Hyper.Services.Authentication
{
    ...
    public class AuthenticationMiddleware
    {
            ...
            IAuthenticationHandlerProvider authenticationHandlerProvider)
        {
            ...
            _authenticationHandlerProvider = authenticationHandlerProvider ?? throw new ArgumentNullException(nameof(authenticationHandlerProvider));
        }
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            foreach (var scheme in await Schemes.GetRequestHandlerSchemesAsync())
            {
                try
                {
                    if (await _authenticationHandlerProvider.GetHandlerAsync(context, scheme.Name) is IAuthenticationRequestHandler handler && await handler.HandleRequestAsync())
                        return;
                }
                catch (Exception)
                {
